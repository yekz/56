		...
		<div class="container">
			...
			<p>
				<span  class="pull-left">
					...
					<span> 发布于 <?php echo (date('Y-m-d H:i',$info["create_time"])); ?></span>
				</span>
				<span class="pull-right">
					<?php $prev = D('Document')->prev($info); if(!empty($prev)): ?><a href="<?php echo U('?id='.$prev['id']);?>">上一个</a><?php endif; ?>
                    <?php $next = D('Document')->next($info); if(!empty($next)): ?><a href="<?php echo U('?id='.$next['id']);?>">下一个</a><?php endif; ?>
				</span>
			</p>
		</div>
	</header>
    ...
    <div class="span9 main-content">
        ...
        <section id="contents">
        	<div class="span9">
        		...
        		<div class="span3">
        			<img src="<?php echo ($info["pic"]); ?>" alt="<?php echo ($info["title"]); ?>" class="img-rounded">
        		</div>
        		<div class="span5">
        			<p><?php if(($info["secondhand"]) == "1"): ?>（二手）<?php endif; ?> <span class="text-error lead"><?php echo ($info["title"]); ?></span></p>
        			<p>价格：<span class="text-success lead">¥ <?php echo ($info["price"]); ?></span></p>
                    <p>库存：<span class="text-info lead"><?php echo ($info["stock"]); ?></span></p>
                    <p>
                    <form action="" id="goodsNumber">
                    数量：
                    <select class="input-small" name="number">
                    <?php if(($info["secondhand"]) == "1"): ?><option>1</option>
                    <?php else: ?>
                    <option>1</option>
                    <option>2</option>
                    <option>3</option>
...
<!-- Baidu Button END -->
        ...
        <?php echo hooks('documentDetailAfter');?>
...
$("#addToCartError").hide();
	...
	$("#addToCart").click(function(){
		$.post("<?php echo U('Cart/add?gid='. $info['id']);?>", $("#goodsNumber").serialize(), success, "json");
		return false;
