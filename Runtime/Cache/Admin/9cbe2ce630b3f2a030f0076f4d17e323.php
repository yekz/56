...
<body>
 ...
 <h3>
    ...
    <div class="data-table">
        <?php
 $thead = array( '_html'=>array( '_th'=>'row-selected', 'th'=>'<input class="check-all" type="checkbox"/>', 'td'=>'<input class="ids" type="checkbox" name="ids[]" value="$id" />', ), 'id'=>'编号', 'title'=>array( '_title'=>'名称', 'tag' =>'a', 'href' =>'Article/index?cate_id=$category_id&pid=$id', ), 'price'=>array( '_title'=>'价格', 'tag' =>'span', ), 'stock'=>array( '_title'=>'库存', 'tag' =>'span', ), 'sold'=>array( '_title'=>'销量', 'tag' =>'span', ), 'uid' =>array( '_title'=>'发布者', 'tag' =>'a', 'func' =>'get_username($uid)', 'href' => 'User/detail?id=$uid', ), 'update_time'=>array( '_title'=>'最后更新', 'tag' =>'span', 'func' => 'date("m-d H:i",$update_time)', ), 'status_text'=>'状态', 'view'=>'浏览', 'comment'=>'评论', '操作'=>array( '编辑'=>array( 'href' => 'Article/edit?cate_id=$category_id&id=$id',), '禁用'=>array( 'href' => 'Article/setStatus?status=1&ids=$id', 'class'=>'ajax-get', 'condition'=>'$status==2',), '禁用'=>array( 'href' => 'Article/setStatus?status=0&ids=$id', 'class'=>'ajax-get', 'condition'=>'$status==1',), '启用'=>array( 'href' => 'Article/setStatus?status=1&ids=$id', 'class'=>'ajax-get', 'condition'=>'$status==0',), '删除'=>array( 'href' => 'Article/setStatus?status=-1&ids=$id', 'class'=>'confirm ajax-get' ), ), ); echo $_table_list = $__controller__->tableList($list,$thead); ?>
    </div>
        <!-- 分页 -->
    <div class="page">
        <?php echo ($_page); ?>
