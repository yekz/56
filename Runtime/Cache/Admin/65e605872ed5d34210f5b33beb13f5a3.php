...
<body>
    ...
    <div class="data-table table-striped">
        <?php
 $thead = array( '_html'=>array( '_th'=>'row-selected', 'th'=>'<input class="check-all" type="checkbox"/>', 'td'=>'<input class="ids" type="checkbox" name="id[]" value="$uid" />', ), 'uid' => 'UID', 'username' => array( '_title'=>'用户名', 'tag' =>'a', 'href' => 'User/detail?id=$uid', ), 'realname'=>'姓名', 'orderform'=>'下单量', 'last_login_ip'=>array( '_title'=>'最后登陆IP', 'tag' =>'span', 'func' => 'long2ip($last_login_ip)', ), 'last_login_time'=>array( '_title'=>'最后登陆时间', 'tag' =>'span', 'func' => 'date("Y-m-d H:i:s",$last_login_time)', ), 'status_text'=>'状态', '操作'=>array( '禁用'=>array( 'href' => 'User/changeStatus?method=forbidUser&id=$uid', 'class'=>'ajax-get', 'condition'=>'$status==1',), '启用'=>array( 'href' => 'User/changeStatus?method=resumeUser&id=$uid', 'class'=>'ajax-get', 'condition'=>'$status==0',), '删除'=>array( 'href' => 'User/changeStatus?method=deleteUser&id=$uid', 'class'=>'confirm ajax-get' ), '授权'=>array( 'href' => 'AuthManager/group?uid=$uid', 'class'=>'authorize'), ), ); echo $_table_list = $__controller__->tableList($_list,$thead); ?>
	</div>
    <div class="page">
        <?php echo ($_page); ?>
    </div>
