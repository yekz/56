...
class ArticleController extends HomeController {
	...
	public function lists($page = 1){
		...
		/* 获取当前分类列表 */
		$Document = D('Document');
		$list = $Document->page($page, $category['list_row'])->order("id asc")->lists($category['id']);
		if(false === $list){
			$this->error('获取列表数据失败！');
		}
		$Cover = M('Picture');
		foreach ($list as $key => $value) {
			$list[$key]['pic'] = $Cover->where('id='.$value['cover_id'])->getField("path");
		}
		/* 获取模板 */
	...
	public function detail($id = 0, $page = 1){
		...
		$Cover = M('Picture');
		$Category = M("Category");
		$info['pic'] = $Cover->where('id='.$info['cover_id'])->getField("path");
		$info['secondhand'] = $Category->where('id='.$info['category_id'])->getField("secondhand");
		if(!$info){
