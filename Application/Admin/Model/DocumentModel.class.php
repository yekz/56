...
class DocumentModel extends Model{
    ...
    protected $_validate = array(
        ...
        array('price', '/[1-9]+[0-9]{0,4}+(.[0-9]{1,1})?$/', '售价格式不合法，仅允许一位小数', self::VALUE_VALIDATE, 'regex', self::MODEL_BOTH),
        array('title', 'require', '标题不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH),
        array('cover_id', 'require', '您还没有上传封面图片', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH),
    ...
    protected $_auto = array(
        ...
        array('title', 'html2txt', self::MODEL_BOTH, 'function'),
        array('sold', 0, self::MODEL_INSERT),
        array('stock', 1, self::MODEL_INSERT),
        array('type', 2, self::MODEL_INSERT),
        array('description', 'html2txt', self::MODEL_BOTH, 'function'),
     ...
     * @author huajie <banhuajie@163.com>
     */
    public function update($secondHand = false){
        /* 获取数据对象 */
        $data = $this->create();
        if ($secondHand) {
            $data['content'] = strip_tags($data['content'], '<font><div><table><embed><img><pre><hr>');
        }
        if(empty($data)){
            return false;
        }
        $data['sid'] = M("Category")->where('id='.$data['category_id'])->getField("pid");
        /* 添加或新增基础内容 */
        if(empty($data['id'])){ //新增数据
            $id = $this->add(); //添加基础内容
            if(!$id){
                $this->error = '新增商品出错，请检查是否填写完整！';
                return false;
            }
        } else { //更新数据
